...
`HANDOFF.md` 永远只有 4 节、全内联、自包含：**上下文 / 当前进度 / 决策记录 / 踩坑记录**。不探测其他框架（OpenSpec / SuperPowers / planning-with-files），不使用指针引用。模板见仓库内 `HANDOFF-template.md`。

## 何时使用

- 新会话开始 → **read**：读取 HANDOFF.md，恢复任务上下文。
- 会话结束 / 任务完成 → **write** 或 **archive**：固化当前状态。
- 长期停滞后重启 → **write**：重新扫描项目并刷新状态。
...
3. 写入项目根目录 `HANDOFF.md`，严格保留 4 节结构。
4. **保留历史**：现有"决策记录"和"踩坑记录"默认原样保留，将本次会话能确认的新记录追加到顶部；不要因扫描无法推断原因就删除历史记录。
5. **更新状态**：按当前扫描重写"上下文"和"当前进度"；无法确认的信息写"未知"或"无"，不要编造。
6. 旧文件若有 `> 派生自：<文件名>` 行，原样保留；首次生成则不添加该行。
...
2. 只提取：一句话任务、进行中事项、卡点、下一步、最近 1–2 条决策、最近 1–2 条踩坑。
3. 若头部有 `> 派生自：<文件名>`，说明当前任务直接派生自该归档。
4. 输出摘要后不阻塞，等用户指示继续；需要细节时可展开指定章节。

### update — 中途定点补丁

1. 识别要更新的章节：上下文 / 当前进度 / 决策记录 / 踩坑记录。
2. 按规则更新，其他章节原样保留：
